...
setwd("C:/Users/marec/OneDrive/Bureau/bureau_these_theo_confi2/Data/DATA/DONNEES FONCTIONNELLE/DIVERSITE_INDICE_CWM/")
dir(pattern=".csv")[-1]
name <- c("inv_iml","inv_zl","inv_zp","mac","phy","poi")

############################## LISTE CONTENANT TOUS LES INDICES POUR TOUS LES MAILLONS ##############################
# diversité fonctionnelle
list_tab_fun<- map2(dir(pattern=".csv")[-1],
                    name,
                    ~read_csv(.x) %>% 
                      setnames(colnames(.)[-ncol(.)],
                               paste0(.y,"_",str_remove_all(colnames(.)[-ncol(.)],"[:upper:]{1,}_(?=.)"))) %>% 
                      dplyr::select(-c(1,2)) %>% 
                      dplyr::select(code_lac,c(1,2,3,4,5)))
...
# diversité taxo
setwd("C:/Users/marec/OneDrive/Bureau/bureau_these_theo_confi2/Data/DATA/DONNEES FONCTIONNELLE/DIVERSITE_INDICE_TAX/")

list_tab_tax <- map2(dir()[-1],
                     name,
                     ~read_csv(.x) %>% 
...
env_pur <- env[-6]
spatial <- env[[6]]
env_alber <- read_csv("C:/Users/marec/OneDrive/Bureau/bureau_these_theo_confi2/Data/DATA/ENV/ALBER & CHARLI/df_alber.csv") %>% 
  dplyr::select(code_lac = rowname,n_alter_cat,p_alter)
env_pur[[6]]<-env_alber

env_charli <- read_csv("C:/Users/marec/OneDrive/Bureau/bureau_these_theo_confi2/Data/DATA/ENV/ALBER & CHARLI/df_charli.csv") %>% 
...
iml_fun <- list_tab_fun[[2]]
iml_all <- join_all(list(iml_tax,iml_fun),by="code_lac",type="inner")

########## macroinv ###########
inv_tax_zl <- list_tab_tax[[2]]
inv_fun_zl <- list_tab_fun[[2]]
inv_all_zl <- join_all(list(inv_tax_zl,inv_fun_zl),by="code_lac",type="inner")

########## macrophytes ###########
mac_tax <- list_tab_tax[[4]]
mac_fun <- list_tab_fun[[4]]
mac_all <- join_all(list(mac_tax,mac_fun),by="code_lac",type="inner")

########## phytoplancton ###########
phy_tax <- list_tab_tax[[5]]
phy_fun <- list_tab_fun[[5]]
phy_all <- join_all(list(phy_tax,phy_fun),by="code_lac",type="inner")

########## poisson ###########
poi_tax <- list_tab_tax[[6]]
poi_fun <- list_tab_fun[[6]]
poi_all <- join_all(list(poi_tax,poi_fun),by="code_lac",type="inner")
get("poi_all")


chem <- "C:/Users/marec/OneDrive/Bureau/bureau_these_theo_confi2/Data/DATA/DONNEES FONCTIONNELLE/PRET_ANALYSE/"
list_tab <- list("iml_all","inv_all","mac_all","phy_all","poi_all")
map(list_tab, ~write.csv(get(.),paste0(chem,.,".csv"),row.names = FALSE))
iml_all
write.csv(phy_all,paste0(chem,"phy_all.csv"),row.names = FALSE)
